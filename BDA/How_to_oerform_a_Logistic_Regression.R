...
# there are for each variable using the sapply() function which applies 
# the function passed as argument to each column of the dataframe.

sapply(training.data.raw,function(x) sum(is.na(x)))
sapply(training.data.raw, function(x) length(unique(x)))

# A visual take on the missing values might be helpful: 
# the Amelia package has a special plotting function missmap() 
# that will plot your dataset and highlight missing values:
...
# Now we need to account for the other missing values. R can easily deal 
# with them when fitting a generalized linear model by setting a parameter 
# inside the fitting function. However, personally I prefer to replace the 
# NAs   by hand  , when is possible. There are different ways to do this, a 
# typical approach is to replace the missing values with the average, the
# median or the mode of the existing one. I  ll be using the average.
...
# For a better understanding of how R is going to deal with the 
# categorical variables, we can use the contrasts() function. 
# This function will show us how the variables have been dummyfied 
# by R and how to interpret them in a model.
...
# For instance, you can see that in the variable sex, 
# female will be used as the reference. 

# As for the missing values in Embarked, since there are only two, 
# we will discard those two rows (we could also have replaced the missing 
# values with the mode and keep the datapoints).
...
# The training set will be used to fit our model which we will be 
# testing over the testing set.
...
# First of all, we can see that SibSp, Fare and Embarked are not statistically 
# significant. As for the statistically significant variables, 
# sex has the lowest p-value suggesting a strong association of the sex of the 
# passenger with the probability of having survived. The negative coefficient for 
# this predictor suggests that all other variables being equal, the male passenger # is less likely to have survived. Remember that in the logit model the response 
# variable is log odds: ln(odds) = ln(p/(1-p)) = a*x1 + b*x2 +  K + z*xn. Since male # is a dummy variable, being male reduces the log odds by 2.75 while a unit 
# increase in age reduces the log odds by 0.037.
